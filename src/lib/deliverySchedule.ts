...
export function getDeliveryScheduleInfo(orderDate: Date = new Date()): DeliveryScheduleInfo {
  const { year, month, day, hour } = getGuatemalaDateParts(orderDate);
  ...
  return {
    isSameDay,
    formattedDate: formatDeliveryDate(deliveryYear, deliveryMonth, deliveryDay),
  };
}
...
export function buildDeliveryScheduleMessage(orderDate: Date | string = new Date()): string {
  ...
  const { isSameDay, formattedDate } = getDeliveryScheduleInfo(date);
