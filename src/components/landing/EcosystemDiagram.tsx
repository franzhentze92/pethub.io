...
const nodePositions: Record<string, { x: number; y: number; label: string }> = {
  client: { x: 50, y: 15, label: 'Cliente' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-gray-600 mb-8 leading-relaxed">
              Clientes compran a proveedores, refugios conectan adoptantes y todo se sincroniza
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className={`text-xs font-bold px-2 py-1 rounded-md ${fromRole?.gradient ?? ''} ${fromRole?.colorText ?? 'text-white'}`}>
                        {fromRole?.shortLabel}
                      </span>
                      <span className="text-landing-aqua text-sm">→</span>
                      <span className={`text-xs font-bold px-2 py-1 rounded-md ${toRole?.gradient ?? ''} ${toRole?.colorText ?? 'text-white'}`}>
                        {toRole?.shortLabel}
                      </span>
                    </div>
          ...
          <div className="relative aspect-square max-w-md mx-auto w-full">
            <svg viewBox="0 0 100 100" className="w-full h-full">
              ...
              <circle cx="50" cy="50" r="8" fill="#00F0C8" className="animate-pulse-glow" />
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none">
              <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-landing-aqua flex items-center justify-center shadow-xl animate-pulse-glow">
