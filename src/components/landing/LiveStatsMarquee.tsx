...
export const LiveStatsMarquee: React.FC = () => {
  ...
  return (
    <section className="relative py-3.5 bg-gray-900 border-y border-landing-aqua/20 overflow-hidden">
      <div className="absolute inset-y-0 left-0 w-16 md:w-24 bg-gradient-to-r from-gray-900 to-transparent z-10" />
      <div className="absolute inset-y-0 right-0 w-16 md:w-24 bg-gradient-to-l from-gray-900 to-transparent z-10" />
      ...
      <div className="flex animate-marquee whitespace-nowrap will-change-transform items-center">
        {items.map((mod, i) => (
          <div key={`${mod.title}-${i}`} className="flex items-center gap-2.5 mx-5 md:mx-8 shrink-0">
            <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${mod.gradient} flex items-center justify-center shadow-sm`}>
              <mod.icon className="w-4 h-4 text-white" />
            </div>
            <span className="text-sm md:text-base font-semibold bg-gradient-to-r from-landing-aqua to-landing-mango bg-clip-text text-transparent">
              {mod.title}
            </span>
            {mod.tag && (
