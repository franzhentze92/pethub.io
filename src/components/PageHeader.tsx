...
import {
  plainPageAccentHeader,
  plainPageAccentHeaderForeground,
...
const PageHeader: React.FC<PageHeaderProps> = ({
  ...
  const fg = plainPageAccentHeaderForeground[accent];
  ...
  return (
    <div
      className={cn(
        ...
        isSolid
          ? cn(plainPageAccentHeader[accent], 'shadow-sm')
          : cn(`bg-gradient-to-r ${gradient}`, 'text-white'),
      )}
    >
      {!isSolid && (
        <div
          aria-hidden
          className="pointer-events-none absolute inset-0 bg-gradient-to-br from-black/10 via-black/5 to-black/20"
        />
      )}
        ...
        <div className="min-w-0 flex-1">
          <h2
            className={cn(
              ...
              isSolid ? fg : '[text-shadow:0_1px_3px_rgba(0,0,0,0.25)]',
            )}
          >
            {title}
          </h2>
          <p
            className={cn(
              'text-sm mt-1 leading-snug font-medium',
              isSolid ? cn(fg, 'opacity-80') : '[text-shadow:0_1px_3px_rgba(0,0,0,0.35)]',
            )}
          >
            {subtitle}
          </p>
